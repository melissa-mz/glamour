...
<!-- HERO SANS background-attachment:fixed; -->
<section class="cat-hero-wow" style="<?php echo $hero_bg ? 'background-image:url(' . $hero_bg . ');background-size:cover;background-position:center;' : ''; ?>">
  <div class="container">
...
.modal-box { background: #fff; padding: 40px; border-radius: 24px; max-width: 440px; width: 90%; box-shadow: 0 30px 80px rgba(0,0,0,0.2); position: relative; animation: modalFade 0.3s ease; }
@keyframes modalFade { from { opacity: 0; transform: translateY(20px) scale(0.95); } to { opacity: 1; transform: translateY(0) scale(1); } }
.modal-close { position: absolute; top: 16px; right: 20px; font-size: 28px; cursor: pointer; color: var(--gris); transition: color 0.3s; }
.modal-close:hover { color: var(--charbon); }
.modal-box h3 { font-family: 'Jost', sans-serif; font-size: 1.6rem; font-weight: 400; margin-bottom: 4px; color: var(--charbon); }
.form-group { margin-bottom: 14px; }
.form-group input, .form-group select { width: 100%; padding: 14px 16px; border: 1px solid #e0e0e0; border-radius: 12px; font-size: 0.95rem; font-family: 'Work Sans', sans-serif; background: #fafafa; transition: border-color 0.3s; outline: none; }
.form-group input:focus, .form-group select:focus { border-color: var(--bordeaux); background: #fff; }
.modal-submit-btn { width: 100%; padding: 16px; background: var(--bordeaux); color: #fff; border: none; border-radius: 12px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: background 0.3s; margin-top: 6px; }
.modal-submit-btn:hover { background: var(--or); }
@media (max-width: 480px) { .modal-box { padding: 30px 24px; } .modal-box h3 { font-size: 1.3rem; } }
</style>
